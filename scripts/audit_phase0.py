...
Inventories every tracked ``.rds`` input under ``cross-section/data`` and
...
def collect_summaries() -> tuple[list[FrameSummary], dict[Path, dict[str, pd.DataFrame]]]:
    ...
    for d in RDS_DIRS:
        if not d.is_dir():
            ...
            continue
        for rds in sorted(d.glob("*.rds")):
            try:
                objs = _load(rds)
            except Exception as exc:
                print(f"[ERROR] cannot read {rds}: {exc}", file=sys.stderr)
                continue
            if not objs:
                print(f"[WARN] no DataFrame in {rds}", file=sys.stderr)
                continue
            raw[rds] = objs
            for name, df in objs.items():
                display = name if name else rds.stem
                summaries.append(summarize_frame(df, rds, display))
...
def render_report(
    ...
    lines.append(
        ...
        "the tracked `.rds` inputs or contract paths."
    )
    lines.append("")

    # 1. Inventory summary
    lines.append("## 1. RDS inventory")
    lines.append("")
