# Option B — Visual deck + notebook report fallback
...
The technical depth lives in `outputs/report/final_presentation_report.html`
and `outputs/report/report.html`, which are generated by the same Docker run.
...
1. Lab laptop has run the public Docker image with the README command.
   Generated files are in `outputs/report/`.
2. Browser fullscreen on `outputs/report/slides_visual.html` (F11).
3. `outputs/report/final_presentation_report.html` and
   `outputs/report/report.html` are open as fallback evidence windows.
4. Terminal remains visible enough to show the completed Docker command.
...
2. **Then run the Docker pipeline** for real:
   ...
   → cross-section → panel → manifest check → Quarto render → notebook
   export). Estimated wall time: 2-3 minutes.
...
3. **When the container exits**, list the produced files:
   ...
   Point at `final_presentation_report.html`, `report.html`, and
   `slides_visual.html`. Open `outputs/report/final_presentation_report.html`
   in a new browser tab and scroll quickly through the step-by-step result
   summary, then close it and return to slides.
